...
new (class ProtocolLoadFallbackHandler {
  ...
  makeVirtualURI(uri){
    let cached = URIMapCacher.getURL(uri);
    if(cached)
    ...
    let doit = ()=>{
      ...
      && navigator.serviceWorker && navigator.serviceWorker.controller
        ...
        return serviceWorkerPrefix + btoa(uri);
      }else{
        ...
        return this.getHandler(scheme).then(handler=>{
          ...
          if(!e.soft || !fallbackProxyAvailable)
            throw e;
    ...
    var res = doit();
    URIMapCacher.set(uri,res);
    return Promise.resolve(res);
  }
  getURIfromURL(url){
    if(fallbackProxyAvailable && uri.startsWith(proxybase))
      return url.substr(proxybase);
    if( serviceWorkerDoesHandleLoadFallbackHandlers
     && navigator.serviceWorker
     && navigator.serviceWorker.controller
     && uri.startsWith(serviceWorkerPrefix)
    ) return url.substr(serviceWorkerPrefix);
    if(this.isURIexempt(url))
      return url;
    return URIMapCacher.getURI(url) || url;
  }
  getCheckVirtualImageURL(uri){
    if(this.isURIexempt(uri))
      return uri;
    var res = URIMapCacher.getURL(uri);
    if(res) return res;
    return new Promise(resolve=>{
      var image = new Image();
      image.$ignorefallbackhandler = true;
      image.src = uri;
      var resolved = false;
      let finish = result => {
        if(resolved)
        ...
        resolved = true;
        if(!result){
          resolve(this.makeVirtualURI(uri));
        }else{
          resolve(uri);
        }
      };
  ...
  get XMLHttpRequest(){
    ...
    var XMLHttpRequest = class XMLHttpRequest extends EventTarget {
      ...
      [onload](event){
        ...
        let p = this[priv];
        if(p.native){
          if(p.xhr.response && p.xhr.response instanceof Document)
            sourceIntercepter.checkDocument(p.xhr.response);
          this.dispatchEvent(event);
        }
      }
      [onloadstart](event){
        ...
        function finish(response){
          ...
          res.then(function(result){
            if(result && result instanceof Document)
              sourceIntercepter.checkDocument(result);
            p.response = result;
            var progress = {
      ...
      open(method, url, _async, user, password){
        var p = this[priv];
        p.request = {
          method, url, "async": _async,
          user, password
        };
        p.xhr.open(method, url, _async, user, password);
        p.status = 0;
        p.statusText = "";
  ...
  onerror(event){
    ...
    if(event.target instanceof HTMLMediaElement){
      let ple = event.target.lasterror && event.target.lasterror.mediaerror || null;
      // A few browsers may produce errors in media elements unrelated to the media they load or play,
      // google chrome for example produces one when it fails to load the poster image.
      if(!event.target.error || event.target.error == ple){
      ...
      if(!event.message)
        event.message = event.target.error.message;
    }
    if(event.target.$ignorefallbackhandler)
      return true;
    ...
    if(event.target instanceof HTMLMediaElement)
      event.target.lasterror.mediaerror = event.target.error;
